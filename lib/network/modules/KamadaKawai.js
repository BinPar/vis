 ...
 */
class KamadaKawai {
  constructor(body, edgeLength, edgeStrength, maxIterations) {
    this.body = body;
  ...
  solve(nodesArray, edgesArray, ignoreClusters = false) {
    ...
    let iterations = 0;
    let maxIterations = this.maxIterations || Math.max(1000,Math.min(10*this.body.nodeIndices.length,6000));
